# Anchor Rules — Analyst Handbook

Deterministic extraction rules for converting raw match evidence into narrative anchors.
...
- Second yellow cards produce a single CARD anchor with `event_type` = CARD and `card_type` documented in `detail`. The `narrative_notes` must state the player was shown a second yellow and dismissed.
...
- *CARD — Direct red, challenge that denies clear goalscoring opportunity. Importance 85.*
- *CARD — Second yellow, tactical foul after earlier caution. Importance 75.*
- *CARD — Yellow, time-wasting at goal kick. Importance 40.*
...
| Player receives second yellow + red | CARD (one anchor). `narrative_notes` states: *"{Player} shown a second yellow card and dismissed."* |
...
8. Is every card differentiated as yellow (40), second-yellow dismissal (75), or direct red (85)?
